...
class PredVisualizationCallback(L.Callback):
    def __init__(self, log_every_n_batches: int, num_samples: int):
        super().__init__()
    ...
    def generate_pred_grid(x: Tensor, y: Tensor, y_hat: Tensor):
        ...
        return make_grid(grid, pad_value=127)
...
class ComputeIoUCallback(L.Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        for class_idx, class_iou in enumerate(iou):
            pl_module.log(f"train_IoU_class_{class_idx}", class_iou)
        pl_module.log("train_mIoU", iou.mean(), prog_bar=True)
    ...
    def on_val_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        for class_idx, class_iou in enumerate(iou):
            pl_module.log(f"val_IoU_class_{class_idx}", class_iou)
        pl_module.log("val_mIoU", iou.mean(), prog_bar=True)
